...
// Chapters listed here will be excluded from the generated MDX registry
...
function discoverLessons() {
  ...
  for (const chapter of chapters) {
    ...
    const files = fs.readdirSync(chapterPath).filter(file => file.endsWith(".mdx"));
    ...
    for (const file of files) {
      ...
      const slug = fileName === chapter.name ? chapter.name : `${chapter.name}/${fileName}`;
...
${lessons.map(({ slug, importPath }) => `  "${slug}": () => import("${importPath}"),`).join("\n")}
}
...
${lessons
  .map(({ slug, importPath, frontmatter }) => {
