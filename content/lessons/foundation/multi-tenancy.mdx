...
<Image src="/learn/images/lessons/foundation/multi-tenancy.svg" alt="Webiny Multi Tenancy" withShadow={false}/>
...
A user account created under the root tenant can access and manage the content of child tenants, while users created under a child tenant can only access and manage their own tenant's data. The root-level users can be also restricted so they can only access specific child tenants or none at all.
