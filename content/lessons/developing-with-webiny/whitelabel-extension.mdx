...
<Callout type="hint" title="Logo Formats">
  The Admin app uses two logo variants: square (for collapsed sidebar) and
  horizontal (for expanded sidebar). You can use the same image for both or
  provide different images for each.
</Callout>
...
1. **Keep assets organized** - Store logos and images in the extension folder
2. **Use PNG or SVG** - These formats work well for logos
3. **Test on multiple screens** - Ensure branding looks good on desktop, tablet, and mobile
4. **Use descriptive names** - Use your company name instead of generic placeholders
5. **Version control assets** - Commit all branding assets to git

<Callout type="hint" title="File Paths">
  When importing assets in extensions, use relative paths from the extension
  file. Webiny's build system handles bundling automatically.
</Callout>
...
The extension system makes it easy to create professional, branded Admin applications. In the next lesson, we'll explore debugging and best practices for Webiny development.
