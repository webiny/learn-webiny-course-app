...
1. **`sdk.cms.listEntries<Product>`** — Calls the Read API to fetch published entries from the `product` model. The `<Product>` generic types the response so `product.values` is fully typed.
