...
While the API Playground uses GraphQL, in your JavaScript/TypeScript applications, you'll use the **Webiny SDK**. This library provides a convenient way to interact with the Headless CMS without writing raw GraphQL queries.

First, you'll need to install the SDK (this is already included in the starter kit):
...
const sdk = new WebinySdk({
  apiToken: "YOUR_API_TOKEN",
  apiHost: "https://your-api-url.com",
  apiTenant: "root" // or your specific tenant
});
```

Here's how the SDK methods map to the GraphQL operations you've just learned:
...
```typescript
const response = await sdk.cms.listEntries({
  modelId: "product",
...
Equivalent to `getProduct`:

```typescript
const response = await sdk.cms.getEntry({
  modelId: "product",
  id: "676d123456789" // entryId or full ID
});
```
...
<Callout type="info">
  The SDK simplifies the `where` structure. You don't need to wrap fields in a `values` object -
  simply pass the field name and operator directly.
</Callout>

### Sorting

Equivalent to `sort` parameter:
...
Equivalent to `search` parameter:
