...
In this lesson, you'll use the API Playground to read and explore content from your Product and Product Category models. You'll learn:

- How to write your first queries in the API Playground
- The difference in how Manage API vs Read/Preview APIs handle reference fields
- How to filter and search for specific records
- How to handle errors in GraphQL queries
...
<Callout type="hint">
  If you haven't created any content yet, go to the Webiny Admin → Headless CMS
  → Product Categories and Products to create and publish a few test entries.
</Callout>
...
<Callout type="info">
  Notice we've included the `error` field in the query. As mentioned in Lesson
  6, this is a best practice for list queries - it helps you catch and debug
...
<Callout type="info">
  The **Read API** only returns **published** content. If you created entries
  but haven't published them yet, they won't appear in this response.
</Callout>
...
<Callout type="warning">
  Notice the difference: - **GraphQL errors**: Top-level `"errors"` array
  (plural) - these are validation/syntax errors caught by GraphQL before Webiny
  processes the query - **Webiny errors**: Inside the query result as `"error"`
  field (singular) - these are business logic errors from Webiny itself
</Callout>
...
<Callout type="hint">
  Replace `"676d123456789"` with an actual `entryId` from your products. You can
  get entry IDs from the `listProducts` query response or from the Webiny Admin
  UI.
</Callout>
...
<Callout type="warning">
  **Manage API reference limitation**: The Manage API only returns metadata for
  referenced entries. To get the actual field values, you need to make a
  separate query using the referenced `entryId`.
</Callout>
...
<Callout type="info">
  **Note**: Comparison operators like `_gt`, `_gte`, `_lt`, `_lte` are **only
  available for number fields**. Text fields use string-specific operators like
  `_contains` and `_startsWith` instead.
</Callout>

<Callout type="hint">
  Use the **DOCS panel** in the API Playground to explore all available filters
  for your specific content models. Search for `ProductListWhereInput` to see
  all filter options. You can also use **auto-complete** (Ctrl+Space /
  Cmd+Space) inside the `values: {}` object to see available operators for each
  field.
</Callout>
...
<Callout type="info">
  The `search` parameter performs a full-text search across fields marked as
  searchable in your content model definition. It's more powerful than
  `_contains` filters because it searches multiple fields simultaneously.
</Callout>
...
<CodeBlock
  ...
  listProducts(
    limit: 10
    after: "WyIyMDI0LTAxLTE1VDEyOjM0OjU2LjAwMFoiXQ=="
  ) {
    data {
      id
      values {
...
<Callout type="hint">
  The `cursor` is an opaque string - you don't need to understand its format.
  Just pass the cursor value from the previous response to the `after`
  parameter.
</Callout>
...
<CodeBlock
  language="graphql"
  code={`query GetProduct($entryId: ID!) {
...
<Callout type="success">
  Using variables makes queries more maintainable and prevents GraphQL injection
  attacks. It's a best practice, especially when building applications.
</Callout>
...
<CodeBlock language="graphql" code={`# ❌ Bad: Requesting too many fields
{
  listProducts {
    data {
      id
...
<CodeBlock language="graphql" code={`# ❌ Bad: Hardcoded value
{
  getProduct(where: { entryId: "676d123456789" }) {
    data { ... }
  }
...
<Callout type="success">
  The API Playground is your best tool for learning and experimenting with
  Webiny's GraphQL APIs. Use it frequently to test queries before implementing
