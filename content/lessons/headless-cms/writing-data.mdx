...
<Callout type="info">
  In this lesson, we'll use the Manage API to **create** new contact form submissions. We'll explore
  updating and deleting in later lessons.
</Callout>
...
<Callout type="hint">
  The "Submitted At" field will automatically record when the form was submitted. This is useful for
...
```ts title="asd"
import { ModelFactory } from "@webiny/api-headless-cms";
...
<Callout type="success">
  Your ContactSubmission content model is now deployed and ready to use! The code approach makes it
...
The host is the domain part: `https://d1ud9w3i9i7d77.cloudfront.net`. This should match what you already have in `WEBINY_API_HOST`.

### Step 2: Update .env.local

In the previous lesson, we added `WEBINY_API_HOST` to our environment variables. This is used for both reading and writing data, so we don't need to add separate variables for the Manage API!

Your `.env.local` should look like this:

```bash
# API Token (needs both Read and Manage permissions)
...
<Callout type="success">
  Because we're using the SDK, we just need the base **API Host** and the SDK handles the endpoints
  internally. The tenant defaults to `"root"` when not specified.
</Callout>
...
Now let's add a function to submit contact form data to Webiny using the SDK.

We'll reuse the same SDK instance from `lib/webiny.ts`, but we also need to export it from there first.

Update `lib/webiny.ts` to export the SDK:
...
Now create a new file `lib/mutations.ts`:

```typescript
import { sdk } from "./webiny";
...
export async function createContactSubmission(data: ContactFormData): Promise<ContactSubmission> {
  // Use SDK to create a new entry
  const response = await sdk.cms.createEntry({
  ...
  if (response.error) {
    throw new Error(response.error.message || "Failed to create contact submission");
  }
  ...
  return response.data as unknown as ContactSubmission;
}
```

<Callout type="info">
  We're reusing the same SDK instance from `lib/webiny.ts`, which means we only configure the
  connection once and use it throughout the app!
</Callout>
...
3. **Server Action calls SDK function** (`createContactSubmission`)

   - Connects to Webiny Manage API
   - Uses SDK to create entry
